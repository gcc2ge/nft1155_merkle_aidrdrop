...
def get_nft_type(nft_name):
    return str(config_dict[nft_name])
...
def get_proof_for(nft_name, address):
    ...
    tokenURI = int(config_dict[nft_name])
    eligible_tokens = get_tokenURI(
        canonical_address+tokenURI.to_bytes(32, "big"), airdrop_dict)
    print(airdrop_dict)
    if eligible_tokens == "0x0":
        proof = []
    else:
        proof = create_proof(
            get_item(canonical_address+tokenURI.to_bytes(32, "big"), airdrop_dict), airdrop_tree)
    return jsonify(
        {
